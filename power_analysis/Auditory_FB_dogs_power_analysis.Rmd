...
```{r}
set.seed(1)
n.subject <- seq(from=120, to=210, by=8)# number subjects (from 40 to 70 per condition). Edited by LL
...
```
## CHECKS - Prepare data for modelling
```{r}
# checks:
...
#r.effects <- c(abs(qlogis(fb.per)))  = 0.08
r.effects <- 0.14
...
for (i in 1:nrow(all.res)) {
  ...
  # add age and breed (if it should be generated in each loop)
  age <- sample(x = age.range, size = length(unique(xdata$subj.id)), replace = T)
  xdata$age <- as.numeric(age[as.numeric(xdata$subj.id)])
  xdata$z.age <- as.vector(scale(as.numeric(start.data$age))) 
  
  xdata$breed <- sample(x = breed, size = length(unique(xdata$subj.id)), replace = T)

  m.mat <- model.matrix(object = ~ condition+sex+baited.fst+z.age, data = xdata) # create model martix 

  
  coefs <- c(
      "(Intercept)" = log(all.res[i, "know.per"] / (1 - all.res[i, "know.per"])),
      "conditionknowledge" = log(all.res[i, "fb.per"] / (1 - all.res[i, "fb.per"] )) - log(all.res[i, "know.per"] / (1 - all.res[i, "know.per"])),
      "conditionknowledgecontrol" = log(all.res[i, "knctr.per"] / (1 - all.res[i, "knctr.per"] )) - log(all.res[i, "know.per"] / (1 - all.res[i, "know.per"])),
...
xdata$choice <- rbinom(n = nrow(xdata), size = 1, prob = exp(LP) / (1 + exp(LP)))
  ...
  full <- keepWarnings(glmer(choice ~ condition + sex + baited.fst + z.age +
       (1+ sex.c + condition.dummy1.c + condition.dummy2.c + baited.fst.dummy1.c + z.age| breed), 
			 family=binomial, data=xdata, control=contr
    ))
#   # fit null model:                                        #LL: I am not sure why a null model is needed in this case, since we are only interested in the effect of condition.
#   null <- keepWarnings(glmer(choice ~ sex+baited.fst+z.age +
#        (1+ sex.c + baited.fst.dummy1.c + z.age | breed), 
# 			 family=binomial, data=xdata, control=contr
#   ))
  # store results:
  all.res[i, c("icpt", "conditionknow", "conditionknctr", "z.age", "sexM", "baited.fstgrey")] <- fixef (full$value)
...
# all.res[i, "warns.null"] <- nchar(paste(null$warnings, collapse = ""))
  ...
  all.res[i, "lrt.con.know.p"] <- as.data.frame(drop1(full$value, test = "Chisq"))["conditionknow", "Pr(>Chi)"]
  all.res[i, "lrt.con.know.ctr.p"] <- as.data.frame(drop1(full$value, test = "Chisq"))["conditionknctr", "Pr(>Chi)"]
# all.res[i, "lrt.p.con"] <- as.data.frame(drop1(full$value, test = "Chisq"))["condition", "Pr(Chi)"]
  ...
  all.res[i, "lrt.sex.p"] <- as.data.frame(drop1(full$value, test = "Chisq"))["sexM", "Pr(>Chi)"]
  all.res[i, "lrt.baited.fst.p"] <- as.data.frame(drop1(full$value, test = "Chisq"))["baited.fstgrey", "Pr(>Chi)"]
# all.res[i, "full.null.p"] <- as.data.frame(anova(null$value, full$value, test = "Chisq"))[2, "Pr(>Chi)"]
  
   print(c("sim # =",i, " sample size =" ,j))
  }
...
```{r echo=FALSE}
#full model
tapply(X=all.res.all.samples[, "warns.full"]>0, INDEX=all.res.all.samples[, c("n.subject", "hh.per", "hl.per", "ll.per", "r.effect")],
FUN=sum)
#warning codes: 
...
n.converged<- all.res2%>%
    group_by(n.subject, hh.per, hl.per, ll.per, r.effect ) %>% #, r.slope.int
...
# New facet label names for ll.per variable
ll.per.labs <- c("LL: 0.4", "LL: 0.5")
names(ll.per.labs) <- c("0.4", "0.5")

# New facet label names for n.subject variable
n.subject.labs <- c( "N = 48", "N = 60", "N = 72")
names(n.subject.labs) <- c("48", "60", "72")

p.con.power <- ggplot(data = lrt.data2, aes(x=as.factor(hh.per) ,y = proportion.sign.lrt.con, fill=as.factor(hl.per))) +
  ...
  facet_grid(ll.per~n.subject, labeller = labeller(ll.per = ll.per.labs, n.subject = n.subject.labs))
