...
load("power_sim_Auditory_FB_IX.RData") 
```
...
r.slope.condition <- 1  #abs(qlogis(fb.per))-abs(qlogis(know.per))
# r.slope.sex <- 0.4 
...
xdata$choice <- rbinom(n = nrow(xdata), size = 1, prob = exp(LP) / (1 + exp(LP)))
 ...
 all.res[i, c("mean_choice_fb","mean_choice_know","mean_choice_knctr")]<-xdata_agg$mean_choice
...
save.image("power_sim_Auditory_FB_IX.RData")
```
...
FUN=sum)
#warning codes: 
#with 240, subjects, 15 models out of 1000 resulted in warnings. 
```
...
n.converged<- all.res2%>%
    group_by(n.subject, fb.per, know.per, knctr.per, r.effect ) %>% 
  summarise(n.converged=length(lrt.p.con))
n.converged #wih 240 subjects, 985 out of 1000 (almost 99% of) models converged
  ...
  summarise(lrt.p.con.median = median(lrt.p.con), 
          ...
          #  lrt.p.age.median = median(lrt.p.age),
           ...
           # n.sign.lrt.age = length(lrt.p.order[lrt.p.age < 0.05]),
            ...
            mean_choice_know_=mean(mean_choice_know),
...
```{r}
ggplot(lrt.data2, aes(x=n.subject, y=proportion.sign.lrt.con*100)) +
